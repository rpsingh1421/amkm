...
export async function POST (request){
    ...
    try {
        ...
        const { member_email, contact, aadhar_number ,pancard_number, ...otherData } = receivedData;
        ...
        const existingaadhaar = await TeamMemberModel.findOne({ aadhar_number: aadhar_number });
        if (existingaadhaar) {
            return NextResponse.json({status:false,message:`provided aadhar number:${aadhar_number} already registered`},{status:400})
        }
        const existingPanCard = await TeamMemberModel.findOne({ pancard_number: pancard_number });
        if (existingPanCard) {
            return NextResponse.json({status:false,message:`provided pancard number:${pancard_number} already registered`},{status:400})
        }
        // hash password
